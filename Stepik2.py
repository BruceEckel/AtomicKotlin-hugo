...
class StepikLessonID:
    ...
    def __init__(self, lesson_line):
        ...
        self.md_name, self.title = StepikLessonID.match_lesson_name(
            self.lesson_name, self.id
        )
        if self.md_name.endswith(".md"):
            self.internal_title = (atomickotlin / self.md_name).read_text().splitlines()[0].strip()
...
stepik_titles = set(s.title for s in lesson_ids)
markdown_titles = set(md.replace("_", " ") for md in markdown_names)
# pprint.pprint(markdown_titles - stepik_titles)
